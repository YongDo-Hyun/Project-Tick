...
void TestQuaZip::getFileList()
{
	...
	{
		QCOMPARE(static_cast<qint64>(dest.uncompressedSize), srcInfo[dest.name].size());
	}
	// Now test zip64
...
void TestQuaZip::add()
{
	...
	foreach (QString fileName, fileNamesToAdd)
	{
		...
		QVERIFY(testFile.open(QIODevice::WriteOnly, QuaZipNewInfo(fileName, "tmp/" + fileName)));
...
void TestQuaZip::setFileNameCodec_data()
{
	...
	QTest::newRow("russian") << QString::fromUtf8("russian.zip") << (QStringList() << QString::fromUtf8("тест.txt"))
							 << QByteArray("IBM866");
#else
	QTest::newRow("latin1") << QString::fromUtf8("latin1.zip") << (QStringList() << QString("tést.txt"))
							<< QByteArray("Latin1");
#endif
}
...
void TestQuaZip::setFileNameCodec()
{
	...
	if (!createTestArchive(zipName, fileNames, QuazipTextCodec::codecForName(encoding)))
	{
		QFAIL("Can't create test archive");
	}
	QuaZip testZip(zipName);
...
void TestQuaZip::setDataDescriptorWritingEnabled()
{
	...
	QuaZipFile testZipFile(&testZip);
	QVERIFY(testZipFile.open(QIODevice::WriteOnly, QuaZipNewInfo("vegetation_info.xml"), NULL, 0, 0));
	...
	QVERIFY(testZipFile20.open(QIODevice::WriteOnly, QuaZipNewInfo("vegetation_info.xml"), NULL, 0, 0));
