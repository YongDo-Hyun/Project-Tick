...
QuaZIODevicePrivate::QuaZIODevicePrivate(QIODevice* _io, QuaZIODevice* _q) : io(_io), q(_q)
{
...
QuaZIODevice::QuaZIODevice(QIODevice* io, QObject* parent) : QIODevice(parent), d(new QuaZIODevicePrivate(io, this))
{
		...
		while (read < maxSize && d->inBufPos < d->inBufSize)
		{
			...
			switch (inflate(&d->zins, Z_SYNC_FLUSH))
			{
				...
				case Z_BUF_ERROR: // this should never happen, but just in case
					if (!d->zBufError)
					{
						qWarning("Z_BUF_ERROR detected with %d/%d in/out, weird", d->zins.avail_in, d->zins.avail_out);
						d->zBufError = true;
					}
					...
					d->inBufSize += more;
					break;
				default: setErrorString(QString::fromLocal8Bit(d->zins.msg)); return -1;
			}
		}
	}
#ifdef QUAZIP_ZIODEVICE_DEBUG_INPUT
	...
	while (written < maxSize)
	{
		...
		switch (deflate(&d->zouts, Z_NO_FLUSH))
		{
			case Z_OK:
				...
				break;
			default: setErrorString(QString::fromLocal8Bit(d->zouts.msg)); return -1;
		}
		if (d->doFlush(error) == -1)
		{
			setErrorString(error);
