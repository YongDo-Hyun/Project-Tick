...
QString GameType::toTranslatedString() const
{
	switch (type)
	{
		case GameType::Survival: return QCoreApplication::translate("GameType", "Survival");
		case GameType::Creative: return QCoreApplication::translate("GameType", "Creative");
		case GameType::Adventure: return QCoreApplication::translate("GameType", "Adventure");
		case GameType::Spectator: return QCoreApplication::translate("GameType", "Spectator");
		default: break;
	}
	if (original)
	{
...
void World::readFromFS(const QFileInfo& file)
{
	...
	if (m_randomSeed == 0)
	{
		auto worldGenBytes = getWorldGenDataFromFS(file);
		if (!worldGenBytes.isEmpty())
		{
			m_randomSeed = loadSeed(worldGenBytes);
		}
	}
}
