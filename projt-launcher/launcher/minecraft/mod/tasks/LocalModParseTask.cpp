...
namespace ModUtils
{
	...
	ModDetails ReadMCModInfo(QByteArray contents)
	{
		...
		else if (jsonDoc.isObject())
		{
			...
			{
				qWarning() << QString(
								  R"(The value of 'modListVersion' is "%1" (expected "2")! The file may be corrupted.)")
								  .arg(version);
	...
	ModDetails ReadMCModTOML(QByteArray contents)
	{
		...
		// we only really care about the first element, since multiple mods in one file is not supported by us at the
		// moment
		...
		// fix up url.
		if (!homeurl.isEmpty() && !homeurl.startsWith("http://") && !homeurl.startsWith("https://")
			...
			if (object.contains("license"))
			{
				...
				if (license.isArray())
				{
					for (auto l : license.toArray())
					{
						...
						{
							auto obj = l.toObject();
							details.licenses.append(ModLicense(obj.value("name").toString(),
	...
	bool process(Mod& mod, ProcessingLevel level)
	{
		switch (mod.type())
		{
			case ResourceType::FOLDER: return processFolder(mod, level);
			case ResourceType::ZIPFILE: return processZIP(mod, level);
			case ResourceType::LITEMOD: return processLitemod(mod);
			default: qWarning() << "Invalid type for mod parse task!"; return false;
		}
	}
	...
	bool processZIP(Mod& mod, [[maybe_unused]] ProcessingLevel level)
	{
		...
		if (zip.setCurrentFile("META-INF/mods.toml") || zip.setCurrentFile("META-INF/neoforge.mods.toml"))
		{
			...
			if (details.version == "${file.jarVersion}")
			{
				if (zip.setCurrentFile("META-INF/MANIFEST.MF"))
				{
					...
					// some mods use ${projectversion} in their build.gradle, causing this mess to show up in
					// MANIFEST.MF also keep with forge's behavior of setting the version to "NONE" if none is found
		...
		else if (zip.setCurrentFile("META-INF/nil/mappings.json"))
		{
			...
			{
				// nilmods can shade nilloader to be able to run as a standalone agent - which includes nilloader's own
				// meta file
	...
	bool loadIconFile(const Mod& mod, QPixmap* pixmap)
	{
		...
		switch (mod.type())
		{
			...
			case ResourceType::ZIPFILE:
			{
				...
				return png_invalid("Failed to set '" + mod.iconPath() + "' as current file in zip archive"); // could
																											 // not set
																											 // icon as
																											 // current
																											 // file.
			...
			default: return png_invalid("Invalid type for mod, can not load icon.");
		}
	}
