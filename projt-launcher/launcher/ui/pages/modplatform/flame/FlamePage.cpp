...
void FlamePage::triggerSearch()
{
	...
	m_listModel->searchWithTerm(m_ui->searchEdit->text(),
								m_ui->sortByBox->currentIndex(),
								m_filterWidget->getFilter(),
								filterChanged);
...
void FlamePage::onSelectionChanged(QModelIndex curr, [[maybe_unused]] QModelIndex prev)
{
	...
	if (!m_current->versionsLoaded || m_filterWidget->changed())
	{
		...
		auto netJob =
			api.getProjectVersions({ m_current, {}, {}, ModPlatform::ResourceType::Modpack }, std::move(callbacks));
...
void FlamePage::suggestCurrent()
{
	...
	m_dialog->setSuggestedPack(m_current->name,
							   new InstanceImportTask(version.downloadUrl, this, std::move(extra_info)));
	QString editedLogoName = "curseforge_" + m_current->logoName;
	m_listModel->getLogo(m_current->logoName,
						 m_current->logoUrl,
...
void FlamePage::onVersionSelectionChanged(int index)
{
	...
	Q_ASSERT(m_current->versions.at(m_selected_version_index).downloadUrl
			 == m_ui->versionSelectionBox->currentData().toString());
...
void FlamePage::createFilterWidget()
{
	...
	connect(m_ui->filterButton,
			&QPushButton::clicked,
			this,
			[this] { m_filterWidget->setHidden(!m_filterWidget->isHidden()); });
