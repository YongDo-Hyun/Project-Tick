...
namespace Modrinth
{
										  ...
										  bool filterChanged)
	{
		...
		if (m_currentSearchTerm == term && m_currentSearchTerm.isNull() == term.isNull() && m_currentSort == sort_str
			&& !filterChanged)
		{
			return;
		}
	...
	void ModpackListModel::getLogo(const QString& logo, const QString& logoUrl, LogoCallback callback)
	{
		if (m_logoMap.contains(logo))
		{
			callback(APPLICATION->metacache()
						 ->resolveEntry(m_parent->metaEntryBase(), QString("logos/%1").arg(logo))
						 ->getFullPath());
		}
		else
		{
	...
	void ModpackListModel::requestLogo(QString logo, QString url)
	{
		...
		MetaEntryPtr entry =
			APPLICATION->metacache()->resolveEntry(m_parent->metaEntryBase(), QString("logos/%1").arg(logo));
		auto job =
			new NetJob(QString("%1 Icon Download %2").arg(m_parent->debugName()).arg(logo), APPLICATION->network());
