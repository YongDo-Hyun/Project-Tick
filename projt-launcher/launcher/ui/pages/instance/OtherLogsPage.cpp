	  ...
	  m_logSearchPaths(instance ? instance->getLogFileSearchPaths() : QStringList{ "logs" })
{
	...
	if (m_instance)
	{
		...
		m_model->setOverflowMessage(
			tr("Cannot display this log since the log length surpassed %1 lines.").arg(m_model->getMaxLines()));
	}
	else
	{
		modelStateToUI();
...
void OtherLogsPage::reload()
{
	...
	if (!file.open(QFile::ReadOnly))
	{
		...
		QMessageBox::critical(this,
							  tr("Error"),
							  tr("Unable to open %1 for reading: %2").arg(m_currentFile, file.errorString()));
	}
	else
	{
		...
		{
			QString line;
			auto error = GZip::readGzFileByBlocks(&file,
												  [&line, handleLine](const QByteArray& d)
												  {
													  auto block	   = d;
