	  ...
	  m_model(model)
{
	...
	ui->actionUpdateItem->setToolTip(
		tr("Try to check or update all selected data packs (all data packs if none are selected)"));
...
void DataPackPage::downloadDialogFinished(int result)
{
	if (result)
	{
		auto tasks = new ConcurrentTask(tr("Download Data Packs"),
										APPLICATION->settings()->get("NumberOfConcurrentDownloads").toInt());
		connect(tasks,
				&Task::failed,
				[this, tasks](QString reason)
				{
...
void DataPackPage::updateDataPacks()
{
	...
	ResourceUpdateDialog update_dialog(this,
									   m_instance,
									   m_model,
									   mods_list,
									   false,
									   { ModPlatform::ModLoaderType::DataPack });
	...
	if (update_dialog.aborted())
	{
		CustomMessageBox::selectable(this,
									 tr("Aborted"),
									 tr("The data pack updater was aborted!"),
									 QMessageBox::Warning)
			->show();
		return;
	}
	...
	{
		auto tasks = new ConcurrentTask("Download Data Packs",
										APPLICATION->settings()->get("NumberOfConcurrentDownloads").toInt());
		connect(tasks,
				&Task::failed,
				[this, tasks](QString reason)
...
GlobalDataPackPage::GlobalDataPackPage(MinecraftInstance* instance, QWidget* parent)
	: QWidget(parent),
	  m_instance(instance)
{
	...
	connect(instance->settings()->getSetting("GlobalDataPacksEnabled").get(),
	...
	connect(instance->settings()->getSetting("GlobalDataPacksPath").get(),
			&Setting::SettingChanged,
			this,
...
void GlobalDataPackPage::updateContent()
{
	...
	if (shouldDisplay())
	{
		...
		m_underlyingPage->updateExtraInfo = [this](QString id, QString value)
		{ updateExtraInfo(std::move(id), std::move(value)); };
