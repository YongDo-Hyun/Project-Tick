...
struct Language
{
	...
	QString languageName() const
	{
		...
		{
			result = u8"New Zealand English"; // No idea why qt translates this to just english and not to New Zealand
											  // English
		}
	...
	{
		return (key == other.key && file_name == other.file_name && file_size == other.file_size
				&& file_sha1 == other.file_sha1 && translated == other.translated && fuzzy == other.fuzzy
				&& total == other.fuzzy && localFileType == other.localFileType);
	}
	...
	void readIndex(const QString& path, QMap<QString, Language>& languages)
	{
		...
		try
		{
			...
			for (auto iter = langObjs.begin(); iter != langObjs.end(); iter++)
			{
				...
				lang.setTranslationStats(Json::ensureInteger(langObj, "translated", 0),
...
void TranslationsModel::reloadLocalFiles()
{
	...
	std::sort(d->m_languages.begin(),
			  d->m_languages.end(),
			  [this](const Language& a, const Language& b)
			  {
				  if (a.key != b.key)
				  {
...
QVariant TranslationsModel::data(const QModelIndex& index, int role) const
{
	...
	switch (role)
	{
		...
		case Qt::ToolTipRole:
		{
			return tr("%1:\n%2 translated\n%3 fuzzy\n%4 total")
				.arg(lang.key,
					 QString::number(lang.translated),
					 QString::number(lang.fuzzy),
					 QString::number(lang.total));
		}
		case Qt::UserRole: return lang.key;
...
QVariant TranslationsModel::headerData(int section, Qt::Orientation orientation, int role) const
{
	...
	else if (role == Qt::ToolTipRole)
	{
		switch (column)
		{
			...
			case Column::Completeness:
			{
				return tr("Completeness is the percentage of fully translated strings, not counting automatically "
						  "guessed ones.");
			}
		}
	}
...
{
	return std::find_if(d->m_languages.begin(),
						d->m_languages.end(),
						[key](Language& lang) { return lang.key == key; });
}
...
bool TranslationsModel::selectLanguage(QString key)
{
	...
	QLocale::setDefault(QLocale(APPLICATION->settings()->get("UseSystemLocale").toBool()
