Some tools that can be used with uVim:
...
efm_perl.pl:	Perl script to filter error messages from the Perl interpreter
		for use with uVim quickfix mode.
...
vimm:		Shell script to start uVim on a DEC terminal with mouse
		enabled.

vimspell.*:	Shell script for highlighting spelling mistakes.

vim_vs_net.cmd: MS-Windows command file to use uVim with MS Visual Studio 7 and
		later.

xcmdsrv_client.c:  Example for a client program that communicates with a uVim
		   server through the X-Windows interface.

unicode.vim	uVim script to generate tables for src/mbyte.c.
