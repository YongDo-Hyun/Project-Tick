...
class Resource : public QObject
{
  ...
  public:
	...
	auto dateTimeChanged() const -> QDateTime
	{
		return m_changed_date_time;
	}
	auto internal_id() const -> QString
	{
		return m_internal_id;
	}
	auto type() const -> ResourceType
	{
		return m_type;
	}
	bool enabled() const
	...
	auto status() const -> ResourceStatus
	{
		return m_status;
	};
	...
	auto metadata() const -> std::shared_ptr<const Metadata::ModStruct>
	{
		return m_metadata;
	}
	...
	auto shouldResolve() const -> bool
	{
		return !m_is_resolving && !m_is_resolved;
	}
	auto isResolving() const -> bool
	{
	...
	auto resolutionTicket() const -> int
	{
		return m_resolution_ticket;
	}
