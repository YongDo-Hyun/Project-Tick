...
bool ResourceFolderModel::installResource(QString original_path)
{
	...
	switch (resource.type())
	{
		...
		case ResourceType::FOLDER:
		{
			...
			{
				qWarning() << "Copy of folder from" << original_path << "to" << new_path
						   << "has (potentially partially) failed.";
				return false;
			}
...
void ResourceFolderModel::installResourceWithFlameMetadata(QString path, ModPlatform::IndexedVersion& vers)
{
	...
	if (vers.addonId.isValid())
	{
		...
		connect(job.get(),
				&Task::succeeded,
				[response, this, &vers, install, &pack]
				{
...
bool ResourceFolderModel::update()
{
	...
	connect(m_current_update_task.get(),
			&Task::succeeded,
			this,
...
void ResourceFolderModel::resolveResource(Resource::Ptr res)
{
	...
	connect(
		...
		&Task::succeeded,
		this,
		[this, ticket, res] { onParseSucceeded(ticket, res->internal_id()); },
		Qt::ConnectionType::QueuedConnection);
	connect(
		task.get(),
		&Task::failed,
		this,
		[this, ticket, res] { onParseFailed(ticket, res->internal_id()); },
		Qt::ConnectionType::QueuedConnection);
	connect(
		task.get(),
		&Task::finished,
...
Task* ResourceFolderModel::createUpdateTask()
{
	...
	auto task			= new ResourceFolderLoadTask(dir(),
											 index_dir,
											 m_is_indexed,
											 m_first_folder_load,
...
QVariant ResourceFolderModel::data(const QModelIndex& index, int role) const
{
	...
	switch (role)
	{
		case Qt::DisplayRole:
			switch (column)
			{
				case NameColumn: return m_resources[row]->name();
				case DateColumn: return m_resources[row]->dateTimeChanged();
				case ProviderColumn: return m_resources[row]->provider();
				case SizeColumn: return m_resources[row]->sizeStr();
				default: return {};
			}
		case Qt::ToolTipRole:
			if (column == NameColumn)
			{
				if (at(row).isSymLinkUnder(instDirPath()))
				{
					return m_resources[row]->internal_id()
						 + tr("\nWarning: This resource is symbolically linked from elsewhere. Editing it will also "
							  "change the original."
							  "\nCanonical Path: %1")
				...
				{
					return m_resources[row]->internal_id()
						 + tr("\nWarning: This resource is hard linked elsewhere. Editing it will also change the "
							  "original.");
				}
			}
	...
	switch (role)
	{
		case Qt::DisplayRole:
			switch (section)
			{
				...
				case DateColumn:
				case ProviderColumn:
				case SizeColumn: return columnNames().at(section);
				default: return {};
			}
		case Qt::ToolTipRole:
		{
			...
			switch (section)
			{
				case ActiveColumn: return tr("Is the resource enabled?");
				case NameColumn: return tr("The name of the resource.");
				case DateColumn: return tr("The date and time this resource was last changed (or added).");
				case ProviderColumn: return tr("The source provider of the resource.");
				case SizeColumn: return tr("The size of the resource.");
				default: return {};
			}
		}
		default: break;
...
void ResourceFolderModel::saveColumns(QTreeView* tree)
{
	...
	// neither passthrough nor override settings works for this usecase as I need to only set the global when the gate
	// is false
...
void ResourceFolderModel::loadColumns(QTreeView* tree)
{
	...
	connect(gSetting.get(),
			&Setting::SettingChanged,
			tree,
			[this, setVisible, overrideSettingName](const Setting&, QVariant value)
			{
...
QMenu* ResourceFolderModel::createHeaderContextMenu(QTreeView* tree)
{
	...
	{ // action to decide if the visibility is per instance or not
		...
		connect(act,
				&QAction::toggled,
				tree,
				[this, tree, overrideSettingName](bool toggled)
				{
...
bool ResourceFolderModel::ProxyModel::lessThan(const QModelIndex& source_left, const QModelIndex& source_right) const
{
	...
	// we are now guaranteed to have two valid indexes in the same column... we love the provided invariants
	// unconditionally and proceed.
...
void ResourceFolderModel::applyUpdates(QSet<QString>& current_set,
									   QSet<QString>& new_set,
									   QMap<QString, Resource::Ptr>& new_resources)
{
	...
	// add new resources to the end
	{
		...
		if (added_set.size() > 0)
		{
			beginInsertRows(QModelIndex(),
							static_cast<int>(m_resources.size()),
...
{
	auto iter = std::find_if(m_resources.constBegin(),
							 m_resources.constEnd(),
