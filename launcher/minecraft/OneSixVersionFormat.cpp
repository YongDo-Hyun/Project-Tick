...
VersionFilePtr OneSixVersionFormat::versionFileFromJson(const QJsonDocument& doc,
														const QString& filename,
														const bool requireOrder)
{
	...
	else if (!out->minecraftVersion.isEmpty())
	{
		...
		else
		{
			out->addProblem(ProblemSeverity::Error,
							QObject::tr("URL for the main jar could not be determined - Mojang removed the server that "
										"we used as fallback."));
		}
		out->mainJar = lib;
	}
	...
	{
		out->addProblem(ProblemSeverity::Error,
						QObject::tr("Version file contains unsupported element '-minecraftArguments'"));
	}
	if (root.contains("+minecraftArguments"))
	{
		out->addProblem(ProblemSeverity::Error,
