			...
			while (iter != childToId.constEnd())
			{
				...
				if (iface->role() == QAccessible::Cell || iface->role() == QAccessible::ListItem)
				{
					...
					AccessibleInstanceViewItem* cell =
						static_cast<AccessibleInstanceViewItem*>(iface->tableCellInterface());
...
void AccessibleInstanceViewItem::selectCell()
{
	...
	{
		case QAbstractItemView::SelectItems: break;
...
QString AccessibleInstanceViewItem::text(QAccessible::Text t) const
{
	...
	switch (t)
	{
		case QAccessible::Name:
			...
			break;
		case QAccessible::Description: value = model->data(m_index, Qt::AccessibleDescriptionRole).toString(); break;
		default: break;
	}
	return value;
}
