...
class ServersModel : public QAbstractListModel
{
  ...
  public:
		...
		switch (role)
		{
			case Qt::DecorationRole:
			{
				switch (column)
				{
					...
					case 1: return m_servers[row].m_address;
	...
	void queryServersStatus()
	{
		...
		m_currentQueryTask =
			ConcurrentTask::Ptr(new ConcurrentTask("Query servers status",
...
// WARNING: this is here because currentChanged is not accurate when removing rows. the current item needs to be fixed
// up after removal.
...
void ServersPage::on_actionAdd_triggered()
{
	...
	// select the new row
	ui->serversView->selectionModel()->setCurrentIndex(m_model->index(position),
