	  ...
	  m_model(model)
{
	...
	connect(depsDisabled.get(),
			&Setting::SettingChanged,
			this,
			[this](const Setting& setting, const QVariant& value)
...
void ModFolderPage::downloadDialogFinished(int result)
{
	if (result)
	{
		auto tasks = new ConcurrentTask(tr("Download Mods"),
...
void ModFolderPage::updateMods(bool includeDeps)
{
	...
	{
		CustomMessageBox::selectable(this, tr("Aborted"), tr("The mod updater was aborted!"), QMessageBox::Warning)
			->show();
		return;
	}
	...
	{
		auto tasks =
			new ConcurrentTask("Download Mods", APPLICATION->settings()->get("NumberOfConcurrentDownloads").toInt());
		connect(tasks,
				&Task::failed,
				[this, tasks](QString reason)
				{
