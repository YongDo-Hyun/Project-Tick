...
ScreenshotsPage::ScreenshotsPage(QString path, QWidget* parent) : QMainWindow(parent), ui(new Ui::ScreenshotsPage)
{
	...
	// Sorts by modified date instead of creation date because that column is not available and would require
	// subclassing, this should work considering screenshots aren't modified after creation.
...
bool ScreenshotsPage::eventFilter(QObject* obj, QEvent* evt)
{
	...
	switch (keyEvent->key())
	{
		case Qt::Key_Delete: on_actionDelete_triggered(); return true;
		case Qt::Key_F2: on_actionRename_triggered(); return true;
		default: break;
	}
	return QWidget::eventFilter(obj, evt);
}
...
void ScreenshotsPage::on_actionUpload_triggered()
{
	...
	if (selection.size() < 2)
	{
		...
		connect(
			...
			[this](QString reason)
			{
				CustomMessageBox::selectable(this, tr("Failed to upload screenshots!"), reason, QMessageBox::Critical)
					->show();
			});
		connect(job.get(),
				&Task::aborted,
...
void ScreenshotsPage::on_actionCopy_Image_triggered()
{
	...
	// You can only copy one image to the clipboard. In the case of multiple selected files, only the first one gets
	// copied.
...
void ScreenshotsPage::on_actionRename_triggered()
{
	...
	QString pattern = QInputDialog::getText(this,
											...
											tr("Enter a name pattern.\nUse {n} for number, {date} for date, {time} for "
