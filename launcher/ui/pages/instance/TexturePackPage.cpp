	  ...
	  m_model(model)
{
	...
	ui->actionUpdateItem->setToolTip(
		tr("Try to check or update all selected texture packs (all texture packs if none are selected)"));
...
void TexturePackPage::downloadDialogFinished(int result)
{
	if (result)
	{
		auto tasks = new ConcurrentTask("Download Texture Packs",
...
void TexturePackPage::updateTexturePacks()
{
	...
	if (update_dialog.aborted())
	{
		CustomMessageBox::selectable(this,
									 tr("Aborted"),
									 tr("The texture pack updater was aborted!"),
									 QMessageBox::Warning)
			->show();
		return;
	}
