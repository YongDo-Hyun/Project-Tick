	  ...
	  m_model(model)
{
	...
	ui->actionUpdateItem->setToolTip(
		tr("Try to check or update all selected resource packs (all resource packs if none are selected)"));
...
void ResourcePackPage::downloadResourcePacks()
{
	...
	auto* downloadDialog = m_downloadDialog.data();
	if (!downloadDialog)
		return;
	connect(this, &QObject::destroyed, downloadDialog, &QDialog::close);
	connect(downloadDialog, &QDialog::finished, this, &ResourcePackPage::downloadDialogFinished);
	downloadDialog->open();
}
...
void ResourcePackPage::downloadDialogFinished(int result)
{
	if (result)
	{
		auto tasks = new ConcurrentTask("Download Resource Pack",
...
void ResourcePackPage::updateResourcePacks()
{
	...
	if (APPLICATION->settings()->get("ModMetadataDisabled").toBool())
	{
		QMessageBox::critical(this,
							  tr("Error"),
							  tr("Resource pack updates are unavailable when metadata is disabled!"));
		return;
	}
	...
	if (update_dialog.aborted())
	{
		CustomMessageBox::selectable(this,
									 tr("Aborted"),
									 tr("The resource pack updater was aborted!"),
									 QMessageBox::Warning)
			->show();
		return;
	}
	...
	{
		auto tasks = new ConcurrentTask("Download Resource Packs",
										APPLICATION->settings()->get("NumberOfConcurrentDownloads").toInt());
		connect(tasks,
				&Task::failed,
				[this, tasks](QString reason)
...
void ResourcePackPage::changeResourcePackVersion()
{
	...
	connect(this, &QObject::destroyed, downloadDialog, &QDialog::close);
	connect(downloadDialog, &QDialog::finished, this, &ResourcePackPage::downloadDialogFinished);
	downloadDialog->setResourceMetadata(resource.metadata());
	downloadDialog->open();
}
