...
void WorldListPage::on_actionData_Packs_triggered()
{
	...
	dialog->restoreGeometry(
		QByteArray::fromBase64(APPLICATION->settings()->get("DataPackDownloadGeometry").toByteArray()));

	GenericPageProvider provider(dialog->windowTitle());

	provider.addPageCreator(
		[this, folder]
		{
...
void WorldListPage::mceditState(LoggedProcess::State state)
{
	...
	switch (state)
	{
		...
		case LoggedProcess::Starting: return;
...
void WorldListPage::worldChanged([[maybe_unused]] const QModelIndex& current,
...
bool WorldListPage::worldSafetyNagQuestion(const QString& actionType)
{
	if (!isWorldSafe(getSelectedWorld()))
	{
		auto result = QMessageBox::question(
			this,
			actionType,
			tr("Changing a world while Minecraft is running is potentially unsafe.\nDo you wish to proceed?"));
