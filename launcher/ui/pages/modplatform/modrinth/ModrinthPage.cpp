...
void ModrinthPage::onSelectionChanged(QModelIndex curr, [[maybe_unused]] QModelIndex prev)
{
	...
	if (!m_current->versionsLoaded || m_filterWidget->changed())
	{
		...
		auto netJob =
			m_api.getProjectVersions({ m_current, {}, {}, ModPlatform::ResourceType::Modpack }, std::move(callbacks));
...
void ModrinthPage::updateUI()
{
	...
	if (m_current->extraDataLoaded)
	{
		...
		{
			text += "<br><br>"
				  + tr("<b>This project has been archived. It will not receive any further updates unless the author "
					   "decides "
	...
	for (auto& ver : m_current->versions)
	{
		if (ver.addonId == m_selectedVersion)
		{
			...
			m_dialog->setSuggestedPack(m_current->name,
									   ver.version,
									   new InstanceImportTask(ver.downloadUrl, this, std::move(extra_info)));
			QString editedLogoName = "modrinth_" + m_current->logoName;
			m_model->getLogo(m_current->logoName,
							 m_current->logoUrl,
...
void ModrinthPage::triggerSearch()
{
	...
	m_model->searchWithTerm(m_ui->searchEdit->text(),
