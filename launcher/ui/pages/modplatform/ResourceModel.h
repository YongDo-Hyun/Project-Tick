...
namespace ResourceDownload
{
	...
	class ResourceModel : public QAbstractListModel
	{
	  ...
	  public:
		...
		inline auto flags(const QModelIndex& index) const -> Qt::ItemFlags override
		{
		...
		{
			return m_current_search_job && m_current_search_job->isRunning();
		}
		bool hasActiveInfoJob() const
		{
			return m_current_info_job.isRunning();
		}
		Task::Ptr activeSearchJob()
		{
			return hasActiveSearchJob() ? m_current_search_job : nullptr;
		}

		auto getSortingMethods() const
		{
			return m_api->getSortingMethods();
		}

		virtual QVariant getInstalledPackVersion(ModPlatform::IndexedPack::Ptr) const
		{
			return {};
		}
		...
		virtual bool checkFilters(ModPlatform::IndexedPack::Ptr)
		{
			return true;
		}
	  ...
	  protected:
		...
		virtual bool isPackInstalled(ModPlatform::IndexedPack::Ptr) const
		{
			return false;
		}
