...
namespace ResourceDownload
{
	...
	void ResourcePage::updateUi(const QModelIndex& index)
	{
		...
		if (current_pack->extraDataLoaded)
		{
			...
			{
				text += "<br><br>"
					  + tr("<b>This project has been archived. It will not receive any further updates unless the "
						   "author decides "
		...
		m_ui->packDescription->setHtml(StringUtils::htmlListPatch(
			text
			+ (current_pack->extraData.body.isEmpty() ? current_pack->description
													  : markdownToHTML(current_pack->extraData.body))));
		m_ui->packDescription->flush();
	}
	...
	void ResourcePage::onResourceToggle(const QModelIndex& index)
	{
		...
		if (pack->versionsLoaded)
		{
			...
			{
				auto version = std::find_if(pack->versions.begin(),
											pack->versions.end(),
											[this](const ModPlatform::IndexedVersion& version)
				...
				if (version == pack->versions.end())
				{
					auto errorMessage = new QMessageBox(
						...
						tr("No versions for '%1' are available.\nThe author likely blocked third-party launchers.")
							.arg(pack->name),
						QMessageBox::Ok,
						this);
	...
	void ResourcePage::openProject(QVariant projectID)
	{
		...
		connect(m_ui->versionSelectionBox,
				&QComboBox::currentIndexChanged,
				this,
				[this, okBtn](int index)
