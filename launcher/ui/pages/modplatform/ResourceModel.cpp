...
namespace ResourceDownload
{
	...
	auto ResourceModel::data(const QModelIndex& index, int role) const -> QVariant
	{
		...
		switch (role)
		{
			...
			case Qt::DecorationRole:
			{
				if (APPLICATION_DYN)
				{
					if (auto icon_or_none =
							const_cast<ResourceModel*>(this)->getIcon(const_cast<QModelIndex&>(index), pack->logoUrl);
			...
			case Qt::CheckStateRole: return pack->isAnyVersionSelected() ? Qt::Checked : Qt::Unchecked;
			case UserDataTypes::INSTALLED: return this->isPackInstalled(pack);
			default: break;
		}
	...
	std::optional<ResourceAPI::SortingMethod> ResourceModel::getCurrentSortingMethodByIndex() const
	{
		...
		{ // Find sorting method by ID
			...
			auto method			 = std::find_if(sorting_methods.constBegin(),
										sorting_methods.constEnd(),
	...
	std::optional<QIcon> ResourceModel::getIcon(QModelIndex& index, const QUrl& url)
	{
		...
		auto cache_entry = APPLICATION->metacache()->resolveEntry(
			metaEntryBase(),
			QString("logos/%1")
				.arg(QString(QCryptographicHash::hash(url.toEncoded(), QCryptographicHash::Algorithm::Sha1).toHex())));
		auto icon_fetch_action = Net::ApiDownload::makeCached(url, cache_entry);

		auto full_file_path = cache_entry->getFullPath();
		connect(icon_fetch_action.get(),
				&Task::succeeded,
				this,
				[this, url, full_file_path, index]
				{
	...
	void ResourceModel::searchRequestSucceeded(QList<ModPlatform::IndexedPack::Ptr>& newList)
	{
		...
		for (auto pack : newList)
		{
			...
			if (auto sel = std::find_if(m_selected.begin(),
										m_selected.end(),
	...
	void ResourceModel::versionRequestSucceeded(QVector<ModPlatform::IndexedVersion>& doc,
												QVariant pack,
...
#endif
		auto pack = std::find_if(m_packs.begin(),
								 m_packs.end(),
								 [&rem](const ModPlatform::IndexedPack::Ptr i) { return rem == i->name; });
		if (pack == m_packs.end())
		{ // ignore it if is not in the current search
			return;
		}
