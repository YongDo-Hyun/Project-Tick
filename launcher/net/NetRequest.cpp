...
namespace Net
{
	...
	void NetRequest::executeTask()
	{
		...
		switch (m_state)
		{
			case State::Succeeded:
				...
				emit finished();
				return;
			case State::Running: qCDebug(logCat) << getUid().toString() << "Running " << m_url.toString(); break;
			case State::Inactive:
			case State::Failed:
	...
	void NetRequest::onProgress(qint64 bytesReceived, qint64 bytesTotal)
	{
		...
		QString dl_progress = tr("%1 / %2")
								  .arg(StringUtils::humanReadableFileSize(bytesReceived))
								  .arg(StringUtils::humanReadableFileSize(bytesTotal));
	...
	void NetRequest::downloadError(QNetworkReply::NetworkError error)
	{
		...
		else
		{
			...
			if (m_reply)
				qCCritical(logCat) << getUid().toString() << "HTTP Status" << replyStatusCode() << ";error"
								   << errorString();
			m_state = State::Failed;
		}
	}
	...
	void NetRequest::sslErrors(const QList<QSslError>& errors)
	{
		...
		{
			qCCritical(logCat) << getUid().toString() << "Request" << m_url.toString() << "SSL Error #" << i << " : "
							   << error.errorString();
	...
	void NetRequest::downloadFinished()
	{
		...
		if (m_state == State::Succeeded) // pretend to succeed so we continue processing :)
		{
			qCDebug(logCat) << getUid().toString()
							<< "Request failed but we are allowed to proceed:" << m_url.toString();
			m_sink->abort();
			emit succeeded();
			emit finished();
			return;
		...
		if (m_state == State::Running)
		{
			...
			if (m_state == State::Failed)
			{
				qCCritical(logCat) << getUid().toString()
								   << "Failed to process response chunk:" << m_sink->failReason();
			}
